...
if (tituloBanner) {
    setInterval(() => {
        ...
        tituloBanner.innerText = frases[indiceBanner];
    }, 3000);
}
...
function pesquisarLanche() {
    ...
    produtos.forEach(produto => {
        ...
        produto.style.display = texto.includes(pesquisa) ? "block" : "none";
    });
}
...
function atualizarContador() {
    if (total) {
        total.innerText = reclamacoes.length;
    }
}
...
function renderizar() {
    ...
    reclamacoes.forEach((item, index) => {
        ...
        card.innerHTML = `
            ...
            <button class="excluir" onclick="excluirReclamacao(${index})">
                Excluir
            </button>
        `;
...
const formularioAvaliacao = document.getElementById("formularioAvaliacao");
const cardsAvaliacoes = document.getElementById("cardsAvaliacoes");
...
if (abrirAvaliacao) {
    abrirAvaliacao.addEventListener("click", () => {
        formularioAvaliacao.classList.toggle("escondido");
    });
}
...
function atualizarTotalAvaliacoes() {
    if (totalAvaliacoes) {
        totalAvaliacoes.innerText = avaliacoes.length;
    }
}
...
function renderizarAvaliacoes() {
    ...
    avaliacoes.forEach((item, index) => {
        ...
        card.innerHTML = `
            ...
            <p>${item.comentario}</p>
            <button class="excluir" onclick="excluirAvaliacao(${index})">
                Excluir
            </button>
        `;
...
function criarAvaliacao() {
    ...
    const estrelas = document.getElementById("estrelasAvaliacao").value;
    ...
    formularioAvaliacao.classList.add("escondido");
}
